package pe.lacafetalab.shared.infrastructure.hibernate;
...
public final class HibernateConfigurationFactory {
    ...
    public DataSource dataSource(
        ...
        Resource sqlResource = resourceResolver.getResource(String.format(
            "classpath:database/%s.sql",
            databaseName
        ));
        String sqlSentences = new Scanner(sqlResource.getInputStream(), "UTF-8").useDelimiter("\\A").next();

        dataSource.setConnectionInitSqls(new ArrayList<>(Arrays.asList(sqlSentences.split(";"))));
    ...
    private List<String> subdirectoriesFor(String contextName) {
        String path = "./src/" + contextName + "/main/pe/lacafetalab/" + contextName + "/";

        String[] files = new File(path).list((current, name) -> new File(current, name).isDirectory());

        if (null == files) {
            path  = "./main/pe/lacafetalab/" + contextName + "/";
